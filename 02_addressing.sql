...
CREATE OR REPLACE FUNCTION pg6502.addr_immediate()
RETURNS INT AS $$
DECLARE
    v_pc INT;
BEGIN
...
CREATE OR REPLACE FUNCTION pg6502.addr_zero_page()
RETURNS INT AS $$
DECLARE
    v_pc INT;
BEGIN
...
CREATE OR REPLACE FUNCTION pg6502.addr_zero_page_x()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_x INT;
...
CREATE OR REPLACE FUNCTION pg6502.addr_zero_page_y()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_y INT;
BEGIN
    SELECT pc, y INTO v_pc, v_y from pg6502.cpu;
    RETURN (pg6502.mem_read(v_pc + 1) + v_y) & 255;
END
$$ LANGUAGE plpgsql;

-- ABS: absolute: first 2 bytes after opcode form 16 bit addr (LE)
-- eg: LDA $4400 -> load from $4400
CREATE OR REPLACE FUNCTION pg6502.addr_absolute()
RETURNS INT AS $$
DECLARE
    v_pc INT;
BEGIN
...
CREATE OR REPLACE FUNCTION pg6502.addr_absolute_x()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_x  INT;
BEGIN
    SELECT pc, x INTO v_pc, v_x from pg6502.cpu;
    RETURN (pg6502.mem_read16(v_pc + 1) + v_x);
END
$$ LANGUAGE plpgsql;

-- ABS,Y: absolute y: 16 bit addr with Y added.
CREATE OR REPLACE FUNCTION pg6502.addr_absolute_y()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_y  INT;
...
BEGIN
    SELECT pc INTO v_pc from pg6502.cpu;
    v_ptr := pg6502.mem_read16(v_pc + 1);
...
CREATE OR REPLACE FUNCTION pg6502.addr_indirect_x()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_x  INT;
...
BEGIN
    SELECT pc, x INTO v_pc, v_x from pg6502.cpu;
    v_ptr := (pg6502.mem_read(v_pc + 1) + v_x) & 255;
    RETURN pg6502.mem_read16(v_ptr);
END
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION pg6502.addr_indirect_y()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_y  INT;
...
BEGIN
    SELECT pc, y INTO v_pc, v_y from pg6502.cpu;
    v_ptr := pg6502.mem_read16(pg6502.mem_read(v_pc + 1));
    RETURN v_ptr + v_y;
END
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION pg6502.addr_relative()
RETURNS INT AS $$
DECLARE
    v_pc INT;
    v_offset INT;
BEGIN
    SELECT pc INTO v_pc from pg6502.cpu;
    v_offset := pg6502.mem_read(v_pc + 1);
    IF v_offset >= 128 THEN
        v_offset := v_offset - 256;
    END IF;
    RETURN v_pc + 2 + v_offset;
END
$$ LANGUAGE plpgsql;
